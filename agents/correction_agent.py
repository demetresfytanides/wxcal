"""
Bias Correction — supervisor + specialist multi-agent design.

Supervisor orchestrates strategy; IDW and QM specialists own their methods.
All tools close over the CorrectionAgent instance so shared state
(ds_corrected, correction_log, attempt) is updated correctly across calls.
"""
...
_SUPERVISOR_PROMPT = """
You are an expert orchestrator for precipitation bias correction.

You have two specialist agents available as tools:
  idw_correction — tunes IDW parameters via leave-one-out CV then applies
                   a spatially-varying multiplicative ratio field.
  qm_correction  — fits a global quantile mapping to match the model
                   precipitation distribution to observations.

Your workflow:
1. Call diagnose_regime to characterise the precipitation signal.
2. Call get_station_density and get_bias_overview for domain context.
3. Based on signal type:
   spatially_coherent  → call idw_correction first.
                         If RMSE degrades > 5 %, call qm_correction.
   spatially_variable  → call qm_correction first.
                         If RMSE degrades > 5 %, call idw_correction.
4. Each specialist returns JSON metrics.  The "accept_recommendation" field
   is True when RMSE degradation is within the 5 % tolerance.
5. You have up to {max_retries} total correction attempts across both methods.
6. Call finish_correction with the method you are accepting (or rejected=True
   if neither improved performance).

ALWAYS call narrate() before each major step.
""".strip()

_IDW_SPECIALIST_PROMPT = """
You are an IDW bias correction specialist.

Your workflow:
1. Call tune_idw_parameters — finds the best (p, radius_km) via leave-one-out
   cross-validation over all station-day pairs in the period.
2. Call apply_correction with best_p and best_radius_km returned in step 1.
   Do NOT override the CV-selected parameters unless best_loo_rmse is
   unreasonably high (e.g. > 5× the pre-correction RMSE).
3. Return the validation metrics JSON exactly as received from apply_correction.

ALWAYS call narrate() before each step.
""".strip()

_QM_SPECIALIST_PROMPT = """
You are a quantile mapping bias correction specialist.

Your workflow:
1. Call apply_qm_correction.
2. Return the validation metrics JSON exactly as received.

ALWAYS call narrate() before applying the correction.
""".strip()
...
class CorrectionAgent:
    ...
    # ------------------------------------------------------------------
    # Tool factory — all closures over self so state is shared
    # ------------------------------------------------------------------

    def _make_tools(self):
        """Return every tool function. Split into groups by caller below."""
        cfg = self.config
        ...
        def diagnose_regime() -> str:
            """
            Classify the precipitation regime as spatially_coherent or
            spatially_variable.  Returns CV, spatial correlation, wet fraction,
            and a method recommendation.  Call this first.
            """
        ...
        def get_station_density() -> str:
            """Return station density and domain size to help interpret IDW radius."""
            import numpy as np
        ...
        def finish_correction(accepted: bool, reasoning: str) -> str:
            ...
            accepted:  True to keep it, False to discard (return raw model data).
        ...
        def tune_idw_parameters() -> str:
            ...
            radius ∈ {3,5,10,20,30,50,75,100,150,200} km).
            Returns best_p, best_radius_km, best_loo_rmse, and top_5 results.
            """
            import itertools
            import math
        ...
        def apply_correction(idw_power: float, radius_km: float) -> str:
            ...
            Returns validation metrics so the supervisor can decide to accept or retry.
            """
            self.attempt += 1
        ...
        def apply_qm_correction() -> str:
            ...
            distribution at station locations.
            Returns validation metrics so the supervisor can decide to accept or retry.
            """
            self.attempt += 1
            if self.attempt > cfg.max_correction_retries:
        ...
        return {
            "supervisor": [narrate, diagnose_regime, get_station_density,
                           get_bias_overview, finish_correction],
            "idw":        [narrate, tune_idw_parameters, apply_correction],
            "qm":         [narrate, apply_qm_correction],
        }
    ...
    def _build_supervisor(self) -> ReActAgent:
        cfg   = self.config
        tools = self._make_tools()

        def _client():
            return make_client(cfg.llm_model)

        client_s, model_name = _client()

        client_idw, _ = _client()
        idw_specialist = ReActAgent(
            client=client_idw,
            model_name=model_name,
            system_prompt=_IDW_SPECIALIST_PROMPT,
            tools=tools["idw"],
            max_steps=8,
        )

        client_qm, _ = _client()
        qm_specialist = ReActAgent(
            client=client_qm,
            model_name=model_name,
            system_prompt=_QM_SPECIALIST_PROMPT,
            tools=tools["qm"],
            max_steps=5,
        )

        supervisor_tools = tools["supervisor"] + [
            idw_specialist.as_tool(
                name="idw_correction",
                description=(
                    "Run IDW bias correction. Tunes power and radius parameters via "
                    "leave-one-out cross-validation (50 combinations), then applies a "
                    "spatially-varying multiplicative ratio field. Best for spatially "
                    "coherent signals. Returns JSON validation metrics including "
                    "rmse_improvement_pct and accept_recommendation."
                ),
            ),
            qm_specialist.as_tool(
                name="qm_correction",
                description=(
                    "Run quantile mapping bias correction. Fits a global distribution "
                    "mapping from model to observed precipitation at station locations. "
                    "Best for spatially variable or location-uncertain signals. "
                    "Returns JSON validation metrics including rmse_improvement_pct "
                    "and accept_recommendation."
                ),
            ),
        ]
    ...
    def run(self, ds_model: xr.Dataset, df_obs: pd.DataFrame,
        ...
        prompt = (
            ...
            f"Step 1: call diagnose_regime to classify the precipitation signal.\n"
            f"Step 2: call get_station_density and get_bias_overview.\n"
            f"Step 3: delegate to the appropriate specialist(s) per your instructions.\n"
            f"Step 4: call finish_correction with your final decision."
        )

        import asyncio
        asyncio.get_event_loop().run_until_complete(self._supervisor(prompt))
